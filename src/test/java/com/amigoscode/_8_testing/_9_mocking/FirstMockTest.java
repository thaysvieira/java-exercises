...
class FirstMockTest {
    ...
    @Test
    void shouldVerifyOrderServiceWhenChargeAndSave(){
        Order order = new Order("ORD-1", "CUST-1", 99.99);
        when(paymentService.charge("CUST-1", 99.99)).thenReturn(true);
        Order actual = underTest.placeOrder(order);
        verify(paymentService).charge("CUST-1", 99.99);
    ...
    @Test
    void shouldVerifyMethodCallSpecificArg(){
        Order order = new Order("ORD-1", "CUST-1", 49.99);
        when(paymentService.charge("CUST-1", 49.99)).thenReturn(true);
        Order actual = underTest.placeOrder(order);
        verify(paymentService).charge("CUST-1", 49.99);
        verify(repository).save(actual);
    }
}
