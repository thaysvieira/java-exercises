...
public class CountMinMax {
    ...
    public static void main(String[] args) {
        ...
        long countSalary = employees.stream().filter(s -> s.salary() > 80000).count();
        ...
        employees.stream().min(Comparator.comparingDouble(Employee::salary)).ifPresent(e-> System.out.println(e.name()+" - "+e.salary()));
        // TODO: 3 - Find the employee with the highest salary
        //           Use max() with an appropriate comparator
        //           Print the employee's name and salary
        employees.stream().max(Comparator.comparingDouble(Employee::salary)).ifPresent(e-> System.out.println(e.name()+" - "+e.salary()));
        // TODO: 4 - Use the Collectors.counting() collector to count all employees
        //           Print the result
        long totalEmployee = employees.stream().collect(Collectors.counting());
        ...
        collect.forEach((department,list)->{
        ...
        //           Print both results
        Optional<Employee> maxPay = employees.stream().collect(Collectors.maxBy(Comparator.comparingDouble(Employee::salary)));
        System.out.println("highest pay...");
        System.out.println(maxPay);
        Optional<Employee> minPay = employees.stream().collect(Collectors.minBy(Comparator.comparingDouble(Employee::salary)));
        System.out.println("lowest pay...");
